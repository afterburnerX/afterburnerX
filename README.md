...
1. **Database**
   ...
   re-running the whole schema. Both paths are verified to produce an
   identical schema — a fresh import and a migrated database were
...
# unit + integration tests against a real MySQL database
TEST_DB_NAME=afterburnerx_test TEST_DB_USER=root TEST_DB_PASS=secret php tests/run.php
```
...
`tests/run.php` is a small runner; cases are `test(...)` / `integration_test(...)`
blocks using `assertSame()` / `assertTrue()` / `assertFalse()` / `assertNull()`.

- **Unit tests** (`tests/cases/`) cover pure logic: CSRF tokens,
  rate-limit classification, retry backoff, upload MIME validation, SigV4
  host/key derivation, SMTP dot-stuffing.
- **Integration tests** (`tests/cases_integration/`) run the repositories
  ...
  anything else rather than risk pointing at a real database.

Not covered: the live Facebook/Instagram and Claude API calls, which
